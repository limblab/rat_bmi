function [H,v,mcc]=FILMIMO3_tik(X,Y,numlags,numsides,lambda,fs)
% Solves for the nonparametric filters of a MIMO linear
% system using a correlation function based approach.
%
%    USAGE:   [H,vaf,mcc]=filMIMO3_tik(X,Y,numlags,numsides,lambda,fs);
%
%
...
% EJP Feb 1997. Updated IHS Jan 2010.
%
if (nargin<=4), lambda=0; disp('Regularization parameter set to 0'); end
if (nargin<=5), fs=1; disp('Sampling rate set to 1.0'); end
...
% Tikonov Regularization
    gamma = eye(size(PX,1))*lambda;
    H = fs*((PX+gamma'*gamma)\PXY);
